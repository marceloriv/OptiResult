    ...
    >
      ...
      <nav className="flex-1 py-6 px-3 flex flex-col gap-1.5">
        {navItems.map(({ id, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              <Icon size={18} strokeWidth={active ? 2.5 : 1.8} style={{ color: active ? "#FFFFFF" : "rgba(238, 237, 254, 0.65)" }} />
              {!collapsed && (
      ...
      >
        ...
        >
          ...
          {!collapsed && <span style={{ fontSize: "0.88rem", fontWeight: 500 }}>Cerrar sesión</span>}
        </button>

        {/* User */}
        <div className="flex items-center gap-3 px-3 py-2.5 mt-2 rounded-xl"
          style={{ background: "rgba(238, 237, 254, 0.08)", border: "1px solid rgba(255, 255, 255, 0.05)" }}>
          <div
            className="rounded-full shrink-0 flex items-center justify-center font-bold"
            style={{ width: 32, height: 32, background: "var(--primary)", color: "#FFFFFF", fontSize: "0.78rem" }}
          >
            AM
          </div>
          {!collapsed && (
            <div>
              <div style={{ color: "#FFFFFF", fontSize: "0.82rem", fontWeight: 600, lineHeight: 1.2 }}>
                Ana Martínez
              </div>
